...
func main() {
	...
	// Development (local): ../../migrations (relative to services/ticketing-service/cmd)
	...
	go func() {
		log.Printf("🚀 HTTP Server running on port %s (multiplexed)", cfg.Port)
	...
	go func() {
		log.Printf("🚀 gRPC Server running on port %s (multiplexed)", cfg.Port)
	...
	go func() {
		log.Printf("🔀 Multiplexer serving HTTP and gRPC on port %s", cfg.Port)
	...
	<-quit
	log.Println("🛑 Shutting down ticketing service...")
	...
	log.Println("✅ Ticketing service stopped gracefully")
}
